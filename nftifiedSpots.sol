...
contract NFTifiedSpots is TNT721 {

    address public _admin; //needs ability to change owner and ownertwo

    address public _owner; //me

    address public _ownerTwo; //senior

    mapping(uint => uint) public tierPrices;

    uint[] mintableTokens = [0,11,12,13,14,21,22,23,24,31,32,33,34,41,42,43,44,51,52,53,54]; 
    uint[] mintedTokens;

    constructor (string memory name, string memory symbol, string memory uri) public TNT721(name, symbol) { 
        _admin = msg.sender;
        _owner = msg.sender;
        _ownerTwo = msg.sender;
        uint mintIndex = totalSupply();
        _safeMint(msg.sender, mintIndex);
        mintedTokens.push(0);
        _setTokenURI(mintIndex, uri);

        tierPrices[0] = 10000000000000000000; //10 TFuel good for testing
        tierPrices[1] = 15000000000000000000000; //50 TFuel 15000
        tierPrices[2] = 13750000000000000000000; //40 TFuel 13750
        tierPrices[3] = 12500000000000000000000; //30 TFuel 12500
        tierPrices[4] = 11250000000000000000000; //20 TFuel 11250
        tierPrices[5] = 10000000000000000000000; //10 TFuel 10,000
    }

    function setOwnerOne(address newOwner) public {
        require(msg.sender == _admin);
        _owner = newOwner;
    }

    function setOwnerTwo(address newOwner) public {
        require(msg.sender == _admin);
        _ownerTwo = newOwner;
    }


    function mintNew(string memory uri, uint256 token) public payable returns(uint256) {
        //check if it is a mintable token
        require(seeIfTokenIsMintable(token),"This is not a mintable token");
        //fix price check
        ...
        uint256 priceOfToken = tierPrices[tier];
        ...
        if(msg.sender == _owner){
            ...
            require(address(this).balance >= priceOfToken, "Mint Failed, pricing issue"); 
            mintedTokens.push(token);
            _safeMint(msg.sender, token);
            _setTokenURI(token, uri);
            sendBalance();
    ...
    function seeIfTokenIsMintable(uint256 num) internal view returns (bool){
         for (uint i = 0; i < mintableTokens.length; i++) {
            if (mintableTokens[i] == num) {
            return true;
            }
        }
        return false;
    ...
    function setNewTierPrice(uint256 tier, uint256 newPrice) public{
        require(msg.sender == _owner, "You are not allowed to change the floor price");
        tierPrices[tier] = newPrice;
    }

    function addTierPrice(uint256 tier, uint256 price) public {
        require(msg.sender == _owner, "You are not allowed to change the the tiers price");
        tierPrices[tier] = price;
    }

    function getTierPrice(uint256 tier) public view returns(uint){
        if(msg.sender == _owner){
    ...
    function addMintableTokens(uint256 newToken)public {
        require(msg.sender == _owner, "You are not allowed to change the floor price");
        mintableTokens.push(newToken);
    }

    function getMintableTokens()public view returns(uint256[] memory){
    ...
    function sendBalance() public {
        ...
        uint balanceHalf = balance / 2;
        //(bool success, ) = msg.sender.call.value(balance)("");
        (bool success, ) = _owner.call{value: balanceHalf} (" ");
        require(success, "Transfer failed.");

        (bool success2, ) = _ownerTwo.call{value: balanceHalf} (" ");
